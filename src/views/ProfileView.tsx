...
import { ArrowRight, Calendar, ExternalLink, Instagram } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ playerSlug }) => {
  ...
  const isJonathan = player.slug === 'jonathan';
  const accentColor = isJonathan ? '#1E3A8A' : '#244437';
  const accentBg = isJonathan ? 'bg-[#1E3A8A]' : 'bg-[#244437]';
  const accentHover = isJonathan ? 'hover:bg-[#172554]' : 'hover:bg-[#1b342a]';
  const instagramHandle = isJonathan ? '@jonathan.nielsen02' : '@timnielsen00';
  const instagramUrl = isJonathan ? 'https://www.instagram.com/jonathan.nielsen02/' : 'https://www.instagram.com/timnielsen00/';
  ...
  return <div className="bg-[#FAF9F6] min-h-screen pb-16 text-[#202421]">
    <div className="bg-[#ECEAE4] border-b border-[#D9D6CC] py-12 lg:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-[#FAF9F6] p-1 rounded-lg border border-[#D9D6CC]">
            {['jonathan','tim'].map(slug => <button key={slug} onClick={() => { setActiveView(slug); window.scrollTo({top:0,behavior:'smooth'}); }} className={`px-3.5 py-1.5 rounded-md text-xs font-bold uppercase tracking-wider ${player.slug === slug ? (slug === 'jonathan' ? 'bg-[#1E3A8A] text-white' : 'bg-[#244437] text-white') : 'text-[#656A65]'}`}>{slug === 'jonathan' ? 'Jonathan Nielsen' : 'Tim Nielsen'}</button>)}
          </div>
        </div>
        {currentTournament && <a href={getLeaderboardUrl(currentTournament)} target="_blank" rel="noopener noreferrer" className={`hidden sm:inline-flex items-center gap-2 px-3.5 py-1.5 rounded-md ${accentBg} ${accentHover} text-white text-xs font-bold uppercase`}><span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"/>LIVE: {currentTournament.name}<ExternalLink className="w-3.5 h-3.5"/></a>}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div><div className="inline-flex px-3 py-1 rounded-full bg-[#FAF9F6] border border-[#D9D6CC] text-xs font-bold uppercase tracking-widest mb-3" style={{ color: accentColor }}>Nielsen Golf</div><h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-black tracking-tight">{player.display_name}</h1><a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-3 text-sm font-bold hover:underline" style={{ color: accentColor }}><Instagram className="w-4 h-4"/><span>{instagramHandle}</span><ExternalLink className="w-3 h-3"/></a></div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-10">
      <div className="flex items-center gap-2 border-b border-[#D9D6CC] pb-2 overflow-x-auto">
        {[{id:'overview',label:'PROFILE & STATS'},{id:'schedule',label:`SCHEDULE (${(currentTournament?1:0)+preparingTournaments.length+upcomingTournaments.length})`},{id:'results',label:`RESULTS (${completedTournaments.length})`}].map(tab => <button key={tab.id} onClick={() => setProfileTab(tab.id as any)} className={`px-4 py-2.5 rounded-lg text-xs font-extrabold tracking-wider uppercase whitespace-nowrap ${profileTab===tab.id?`${accentBg} text-white`:'text-[#656A65] hover:bg-[#ECEAE4]' }`}>{tab.label}</button>)}
      </div>
